#pragma once


////////////////////////////////////
//
// Type definitions for HIE module
//
////////////////////////////////////


typedef union HIE_tduLinkedObject HIE_tduLinkedObject;
typedef struct HIE_tdstSuperObject HIE_tdstSuperObject;

typedef struct HIE_tdstEngineObject HIE_tdstEngineObject;
typedef struct HIE_tdstSector HIE_tdstSector;

typedef struct HIE_tdstFamilyList HIE_tdstFamilyList;
//typedef struct HIE_tdstAlwaysActiveCharacterList HIE_tdstAlwaysActiveCharacterList;
typedef struct HIE_tdstAlwaysActiveCharacter HIE_tdstAlwaysActiveCharacter;
